...
use crate::types::{CameraFrame, CameraFormat};
...
pub async fn capture_focus_sequence(
    ...
    if config.focus_start < 0.0 || config.focus_start > 1.0 ||
       config.focus_end < 0.0 || config.focus_end > 1.0 {
        return Err(FocusStackError::InvalidConfig(
    ...
    log::info!(
        ...
        config.num_steps, config.focus_start, config.focus_end, config.step_delay_ms
    );
    ...
    for step in 0..config.num_steps {
        ...
        log::debug!("Capturing focus step {}/{} at distance {:.3}", 
            step + 1, config.num_steps, focus_distance);
        ...
        match capture_with_reconnect(
            ...
            Ok(frame) => {
                log::debug!("Captured frame: {}x{} ({} bytes)",
                    frame.width, frame.height, frame.size_bytes);
                frames.push(frame);
            }
            Err(e) => {
                log::error!("Failed to capture frame at step {}: {}", step + 1, e);
                return Err(FocusStackError::MergeFailed(
                    format!("Capture failed at step {}: {}", step + 1, e)
                ));
            }
        }
        ...
        if step < config.num_steps - 1 {
            tokio::time::sleep(
                tokio::time::Duration::from_millis(config.step_delay_ms as u64)
            ).await;
        }
    }
...
pub async fn capture_focus_brackets(
    ...
    log::info!("Capturing {} brackets with {} shots each", brackets, shots_per_bracket);
    ...
    for bracket_idx in 0..brackets {
        ...
        let frames = capture_focus_sequence(
            device_id.clone(),
            config,
            format.clone()
        ).await?;
    ...
    log::info!("Captured total of {} frames across {} brackets", 
        all_frames.len(), brackets);
