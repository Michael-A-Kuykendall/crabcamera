...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "audio")]
    {
        use crabcamera::audio::{list_audio_devices, get_default_audio_device};
        ...
        for (i, dev) in devices.iter().enumerate() {
            ...
            println!("         Sample Rate: {} Hz, Channels: {}", dev.sample_rate, dev.channels);
            println!("         ID: {}", dev.id);
        }
        
        // Look for OBSBOT
        let obsbot = devices.iter().find(|d| d.name.to_lowercase().contains("obsbot"));
        ...
        println!("      {} Hz, {} channel(s)", default.sample_rate, default.channels);
    }
    ...
    #[cfg(feature = "audio")]
    {
        ...
        while start.elapsed() < duration {
            ...
            if let Some(frame) = capture.try_read() {
                ...
                let level: f32 = frame.samples.iter()
                    .map(|s| s.abs())
        ...
        println!("      Duration: {:.2}s of audio", 
            total_pcm_samples as f64 / 48000.0 / 2.0);  // stereo
